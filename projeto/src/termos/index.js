...
export default function App() {
  ...
  const [imagemUrso, setImagemUrso] = useState('');

  function entrar() {
    setNome('foi aceito ✅');
    const randomUrl = Math.random() < 0.5 ? 'https://placebear.com/200/300' : 'https://placebear.com/g/200/300';
    setImagemUrso(randomUrl);
  }
  ...
  return (
    <View style={{ marginTop: 20 }}>
      ...
      {imagemUrso ? (
        <Image
          source={{ uri: imagemUrso }}
          style={styles.imagemUrso}
        />
      ) : null}
    </View>
  );
